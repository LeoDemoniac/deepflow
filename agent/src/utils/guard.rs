...
impl SystemLoadGuard {
    ...
    fn check(
        ...
        {
            let has_exceeded =
                system_load / cpu_count >= system_load_circuit_breaker_threshold * 0.9;
            if has_exceeded {
                ...
                if now > self.last_exceeded + Self::CONTINUOUS_SAFETY_TIME {
                    info!(
                        "Current load {:?} is below the threshold({:?}), set the agent to enabled.",
    ...
    pub fn start(&self) {
        ...
        let thread = thread::Builder::new().name("guard".to_owned()).spawn(move || {
            ...
            loop {
                ...
                system_load.check(config.system_load_circuit_breaker_threshold, config.system_load_circuit_breaker_metric);
