...
func (o OperatorBase[MT]) dedupInDB(items []*MT, lcuuids []string, lcuuidToItem map[string]*MT) ([]*MT, []string, bool) {
	...
	if len(dupItems) != 0 {
		if o.softDelete {
			...
			log.Info(o.org.LogPre("%s data is duplicated with db data (lcuuids: %v, ids: %v), will learn again", o.resourceTypeName, dupLcuuids, dupItemIDs))
			...
			log.Error(o.org.LogPre("%s data is duplicated with db data (lcuuids: %v)", o.resourceTypeName, dupLcuuids))
